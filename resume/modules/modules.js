...
// [modules.js] v2.1 — canvas sections accept data; full-width; star/slider size parity with wizard
console.log('%c[modules.js] v2.1 loaded', 'color:#22c1c3');

import { ensureCanvas, isSidebarActive, getSideMain, getRailHolder, ensureAddAnchor } from '../layouts/layouts.js';
const $  = (s,r)=> (r||document).querySelector(s);
const $$ = (s,r)=> Array.from((r||document).querySelectorAll(s));

/* ---------- CSS (full-width + shared card styles) ---------- */
(function injectModulesCSS(){
  if (document.getElementById('modules-css')) return;
  const css = `
  /* canvas uses the full available width */
  ...
  [data-dark="1"] .title-item{background:#0f1526}
  ...
  /* ===== Skills ===== */
  .grid-skills{display:grid;gap:10px;min-width:0; justify-content:center}
  ...
  .skill{display:grid;grid-template-columns:20px 20px 221px 100px;align-items:center;gap:10px;margin:4px 0;min-width:0}
  .skill .del{display:grid;place-items:center}
  .skill > span[contenteditable]{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:40px}
  .skill .stars{display:inline-flex;gap:6px;justify-content:center}
  ...
  .skill input[type=range]{-webkit-appearance:none;appearance:none;width:100px;height:4px;border-radius:999px;background:linear-gradient(90deg,var(--accent2),var(--accent));justify-self:center}
  .skill input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:14px;height:14px;border-radius:50%;background:var(--accent);border:2px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,.25);cursor:pointer}
  ...
  /* DnD placeholder */
  .drop-ph{border:2px dashed #8da0ff44; border-radius:12px; height:40px !important}
  ...
  [data-dark="1"] .edu-card{background:#0f1324;border-color:#1f2540}
  ...
  .badge{display:inline-block;background:color-mix(in srgb, var(--accent) 18%, #fff);color:color-mix(in srgb, var(--accent) 70%, #000);border:1px solid color-mix(in srgb, var(--accent) 35%, #0000);padding:2px 8px;border-radius:999px;font-weight:700;font-size:12px;white-space:nowrap;max-width:120px;overflow:hidden;text-overflow:ellipsis}
  ...
  [data-dark="1"] .exp-card{background:#0f1324;border-color:#1f2540}
  `;
...
function starRow(label='Skill', active=0){
  ...
  `;
  d.querySelectorAll('.star').forEach((s,ix)=>{ if(ix<active) s.classList.add('active'); s.addEventListener('click', e=>{ const n=+e.currentTarget.dataset.i; d.querySelectorAll('.star').forEach((el,i)=> el.classList.toggle('active', i<n)); }); });
  d.querySelector('.del').onclick = (e)=>{ e.stopPropagation(); d.remove(); };
  return d;
}
...
/** data: optional array [{type:'star'|'slider', label, stars|value}], opts:{toRail:boolean} */
export function renderSkills(data=[], opts={}){
  let secWrap = document.querySelector('.grid-skills')?.closest('.section');
  if (secWrap){ ensureAddAnchor(); return; }
  ...
  const placeInRail = opts.toRail ?? isSidebarActive();
...
export function renderEdu(items=[]){
  let exists = document.querySelector('.edu-grid');
  if (exists){ ensureAddAnchor(); return; }
  ...
  const mkCard = (kind='course', t='Title', d='2018–2022', a='Academy')=>{
    const ic = kind==='degree' ? 'fa-graduation-cap degree' : 'fa-scroll course';
    ...
    c.innerHTML = `
      ...
      <div class="edu-line1"><i class="fa-solid ${ic} edu-title-icon"></i><span class="badge" contenteditable>${d}</span></div>
      <span class="editable" contenteditable>${t}</span>
      <span class="editable edu-academy" contenteditable>${a}</span>`;
  ...
  if(Array.isArray(items) && items.length){
    items.forEach(x=> grid.appendChild(mkCard(x.kind||'course', x.title||'Title', x.dates||'2018–2022', x.academy||'Academy')));
  }
...
/* ===================== EXPERIENCE ===================== */
/** items: optional array of cards to append immediately */
export function renderExp(items=[]){
  let list = document.querySelector('.exp-list');
  if (!list){
    const sec = document.createElement('div'); sec.className='section';
    ...
    list = sec.querySelector('.exp-list');

    const mk = (dates='Jan 2024 – Present',role='Job title',org='@Company',desc='Describe impact, scale and results.')=>{
    ...
    sec.querySelector('[data-add-exp]').onclick = ()=>{ list.appendChild(mk()); attachDnd(list,'.exp-card'); };
    ...
    items.forEach(d=>{
      const mk = (dates,role,org,desc)=>{
        ...
        c.innerHTML = `
          ...
          <div contenteditable>${desc||'Describe impact, scale and results.'}</div>`;
        c.querySelector('.rm').onclick = e=>{ e.stopPropagation(); c.remove(); };
        return c;
      };
      list.appendChild(mk(d.dates, d.role, d.org, d.desc));
    });
    attachDnd(list,'.exp-card');
  }
...
export function renderBio(text=''){
  ...
  body.textContent = text || 'Add a short summary of your profile, strengths and what you’re looking for.';
...
export function openAddMenu(anchorBtn){
  ...
  const already = {
    ...
    exp:    !!document.querySelector('.exp-list'),
