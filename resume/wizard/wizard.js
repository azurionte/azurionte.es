...
/* ---------- styles (adds fixed mock size + overflow) ---------- */
(function ensureWizardStyle(){
  ...
  st.textContent = `
    #wizard .mock{
      position:relative;border:1px solid #1f2540;border-radius:14px;background:#0c1324;cursor:pointer;
      width:450px;height:158px;overflow:hidden;padding:10px;
    ...
    #wizard .swatch.customize{
      ...
      display:grid; place-items:center; font-weight:800; color:#111; text-shadow:0 1px 0 #fff8;
    }
    @keyframes wizRainbow{0%{background-position:0% 50%}100%{background-position:100% 50%}}

    /* Gradient modal */
    #gradModal{position:fixed;inset:0;display:none;place-items:center;z-index:22000;background:rgba(0,0,0,.55)}
    #gradPanel{width:min(560px,94vw);background:#0f1420;color:#e6e8ef;border:1px solid #1f2540;border-radius:16px;padding:18px;box-shadow:0 40px 140px rgba(0,0,0,.6)}
    ...
    .stop{position:absolute;top:50%;transform:translate(-50%,-50%);width:22px;height:22px;border-radius:50%;border:2px solid #fff;box-shadow:0 6px 14px rgba(0,0,0,.35);cursor:grab;background:#cfd6ff}
    #gradSample{height:42px;border-radius:10px;border:1px solid #2b324b;margin-top:12px}
    .gline{display:flex;gap:8px;justify-content:flex-end;margin-top:12px}
    .gline .mbtn{padding:8px 12px}
    #wizard .toast{position:fixed;padding:6px 10px;border-radius:999px;background:#16a34a;color:#fff;font-weight:700;box-shadow:0 10px 24px rgba(0,0,0,.35);opacity:0;transform:translateY(8px);animation:wztoast .9s ease forwards}
    @keyframes wztoast{to{opacity:1;transform:translateY(-6px)}}
  `;
  document.head.appendChild(st);
})();

/* ---------- Public API (unchanged except visuals) ---------- */
...
/* ---------- Wizard internals (unchanged) ---------- */
const STEPS = [
  { k:'layout',label:'Layout' },{ k:'theme',label:'Theme' },{ k:'contact',label:'Profile data' },
  { k:'skills',label:'Skills' },{ k:'education',label:'Education' },{ k:'experience',label:'Experience' },
  { k:'bio',label:'Bio' },{ k:'done',label:'Done' },
];
...
function buildWizard(){
  ...
  STEPS.forEach((s,i)=>{ const el=document.createElement('div'); el.className='step'; el.dataset.i=i;
  ...
  document.getElementById('wizStartOver').onclick=()=>{ Object.assign(S,{contact:{name:'',phone:'',email:'',address:'',linkedin:''},skills:[],edu:[],exp:[],bio:''}); stepIdx=0; backCount=0; renderStep(); };
  document.getElementById('wizNext').onclick=advance;
}
function markSteps(){
  const nodes=[...document.querySelectorAll('#stepList .step')];
  nodes.forEach((el,i)=>{ el.classList.toggle('on',i===stepIdx); el.classList.toggle('done',i<stepIdx);
    el.style.color=i===stepIdx?'#e8ecff':(i<stepIdx?'#a7ffcf':'#c9d1ff80'); el.style.background=i===stepIdx?'#131a31':'transparent';
    const dot=el.querySelector('.dot'); if(dot) dot.style.background=i<stepIdx?'#26d07c':'#2e3b66';
  });
}
function renderStep(){
  const body=document.getElementById('wizBody'); const s=STEPS[stepIdx].k; markSteps();
  ...
  if(s==='layout'){
    ...
    row.addEventListener('click',e=>{ const m=e.target.closest('.mock'); if(!m) return; row.querySelectorAll('.mock').forEach(x=>x.classList.remove('sel')); m.classList.add('sel'); morphTo(m.dataset.layout); });
  ...
  if(s==='theme'){
    body.innerHTML=`
      ...
      <div class="theme-row" style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px">
        ${['coral','sea','city','magentaPurple','magentaPink','blueGreen','grayBlack','custom'].map(k=>k==='custom'
    ...
    body.querySelectorAll('.swatch[data-k]').forEach(swatch=>{ const k=swatch.dataset.k; swatch.onclick=()=>{ document.body.setAttribute('data-theme',k); S.theme=k; restyleContactChips(); };});
    body.querySelector('#customizeSw').onclick=()=>openGradModal();
    body.querySelector('#wizDark').onclick=e=>{ e.currentTarget.classList.toggle('on'); S.dark=e.currentTarget.classList.contains('on'); document.body.setAttribute('data-dark',S.dark?'1':'0'); restyleContactChips(); };
  ...
  if(s==='contact'){
    ...
    ['nm','ph','em','ad','ln'].forEach(id=>{
      body.querySelector('#'+id).oninput=()=>{ S.contact={ name:body.querySelector('#nm').value, phone:body.querySelector('#ph').value, email:body.querySelector('#em').value, address:body.querySelector('#ad').value, linkedin:body.querySelector('#ln').value }; applyContact?.(); };
    });
  }
  ...
  if(s==='skills'){
    ...
    function row(it,idx){
      ...
      d.innerHTML=`<div class="ws-head"><button class="mbtn" data-rm style="padding:6px 10px">×</button> <span class="ws-label" contenteditable>${it.label||'Skill'}</span></div><div class="right"></div>`;
      const R=d.querySelector('.right');
      if(it.type==='star'){
        const box=document.createElement('div'); box.className='ws-stars';
        for(let i=1;i<=5;i++){ const s=document.createElementNS('http://www.w3.org/2000/svg','svg'); s.setAttribute('viewBox','0 0 24 24'); s.classList.add('ws-star'); if(i<=(+it.value||0)) s.classList.add('active'); s.innerHTML='<path d="M12 17.27 18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>'; s.onclick=()=>{ it.value=i; box.querySelectorAll('.ws-star').forEach((e,ix)=>e.classList.toggle('active',ix<i)); }; box.appendChild(s); }
        R.appendChild(box);
      }else{
        const r=document.createElement('input'); r.type='range'; r.min=0; r.max=100; r.value=+it.value||50; r.className='ws-range'; r.oninput=()=>it.value=r.value; R.appendChild(r);
      }
  ...
  if(s==='education'){
    ...
    function card(it,idx){
      ...
      c.innerHTML=`<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><button class="mbtn" data-rm style="padding:6px 10px">×</button><div style="font-weight:700">${icon} <span class="ws-label" contenteditable>${it.title||'Title'}</span></div></div><div style="margin-top:6px"><span class="ws-label" contenteditable>${it.year||'2018–2022'}</span></div><div style="margin-top:6px"><span class="ws-label" contenteditable>${it.academy||'Academy'}</span></div>`;
      const els=c.querySelectorAll('.ws-label'); els[0].oninput=e=>it.title=e.target.textContent; els[1].oninput=e=>it.year=e.target.textContent; els[2].oninput=e=>it.academy=e.target.textContent;
  ...
  if(s==='experience'){
    body.innerHTML=`<div class="wtitle" style="font-weight:900;font-size:18px;margin-bottom:8px">Experience</div><div id="xWrap"></div><div class="k-row"><button class="mbtn" id="addRole">+ Add another</button></div>`;
    const wr=body.querySelector('#xWrap');
    function card(it){
      const c=document.createElement('div'); c.className='sec'; c.style.cssText='background:color-mix(in srgb, var(--accent) 12%, #0c1324);border:1px solid #243057;border-radius:10px;padding:10px;margin:10px 0';
      c.innerHTML=`<div style="display:flex;gap:8px;align-items:center"><span class="badge" contenteditable style="display:inline-block;padding:6px 10px;border-radius:999px;background:color-mix(in srgb, var(--accent) 16%, #fff);border:1px solid color-mix(in srgb, var(--accent) 40%, #0000)">${(it.dates||'Jan 2022').slice(0,16)}</span><span contenteditable style="font-weight:800">${it.role||'Job title'}</span></div><div style="font-weight:700;color:#cbd5e1;margin-top:4px" contenteditable>${it.org||'@Company'}</div><div style="margin-top:6px" contenteditable>${it.desc||'Describe impact, scale and results.'}</div>`;
      const edits=c.querySelectorAll('[contenteditable]'); edits[0].oninput=e=>it.dates=e.target.textContent.slice(0,16); edits[1].oninput=e=>it.role=e.target.textContent; edits[2].oninput=e=>it.org=e.target.textContent; edits[3]?.oninput&&(edits[3].oninput=e=>it.desc=e.target.textContent);
      return c;
    }
    function paint(){ wr.innerHTML=''; S.exp.forEach(it=>wr.appendChild(card(it))); }
    paint();
    body.querySelector('#addRole').onclick=(ev)=>{ S.exp.push({dates:'',role:'',org:'',desc:''}); paint(); const r=ev.currentTarget.getBoundingClientRect(); const t=document.createElement('div'); t.className='toast'; t.textContent='Experience added'; document.body.appendChild(t); t.style.left=(r.left+window.scrollX)+'px'; t.style.top=(r.top+window.scrollY-40)+'px'; setTimeout(()=>t.remove(),900); };
  }

  if(s==='bio'){
    body.innerHTML=`<div class="wtitle" style="font-weight:900;font-size:18px;margin-bottom:8px">Bio</div><textarea id="bioText" class="wipt" rows="6" placeholder="Short profile…" style="width:100%;min-height:120px"></textarea>`;
    const t=body.querySelector('#bioText'); t.value=S.bio||''; t.oninput=()=>{ S.bio=t.value; };
  }
...
function advance(){
  ...
  if(cur==='done'){ document.getElementById('wizard').style.display='none'; document.getElementById('welcome')?.remove(); document.getElementById('canvasAdd')?.style&&(document.getElementById('canvasAdd').style.display='flex'); return; }
  if(stepIdx<STEPS.length-1){ stepIdx++; renderStep(); }
}

/* ---------- helpers (ONLY mock visuals changed) ---------- */
...
function mock(layoutKey){
    ...
    return `
      <div class="mock sidebar" data-layout="${layoutKey}">
        ...
        <div class="txt"  style="position:absolute;left:40%;right:20px;top:24px;display:grid;gap:8px">
          <div style="height:10px;border-radius:999px;background:#2b375f;width:58%"></div>
          <div style="height:10px;border-radius:999px;background:#2b375f;width:42%"></div>
          <div style="height:10px;border-radius:999px;background:#2b375f;width:64%"></div>
        </div>
      </div>`;
  }
    ...
    return `
      <div class="mock fancy" data-layout="${layoutKey}">
        ...
        <div class="txt"  style="position:absolute;left:24px;right:24px;top:118px;display:grid;gap:10px">
          ...
          <div style="height:10px;border-radius:999px;background:#2b375f;width:46%;margin:0 auto"></div>
        </div>
      </div>`;
  }
  ...
  return `
    <div class="mock top" data-layout="${layoutKey}">
      <div class="hero" style="height:74px;margin:10px;border-radius:14px;background:linear-gradient(135deg,#5b6fb7,#2f3d7a)"></div>
      <div class="pp"   style="position:absolute;right:24px;top:22px;width:52px;height:52px;border-radius:50%;background:#cfd6ff;border:3px solid #fff;box-shadow:0 8px 20px rgba(0,0,0,.35)"></div>
...
/* ---------- Gradient editor (unchanged) ---------- */
let gState={ a:'#8b5cf6', b:'#d946ef', p1:20, p2:80 }, gWired=false;
function openGradModal(){ const o=document.getElementById('gradModal'); if(!o) return; const cs=getComputedStyle(document.documentElement); gState.a=S.custom?.a||cs.getPropertyValue('--accent').trim()||gState.a; gState.b=S.custom?.b||cs.getPropertyValue('--accent2').trim()||gState.b; updateGradSample(); o.style.display='grid'; o.setAttribute('aria-hidden','false'); }
function closeGradModal(){ const o=document.getElementById('gradModal'); if(!o) return; o.style.display='none'; o.setAttribute('aria-hidden','true'); }
function setGradientVars(a,b){ document.documentElement.style.setProperty('--accent',a); document.documentElement.style.setProperty('--accent2',b); S.theme='custom'; S.custom={a,b}; restyleContactChips?.(); }
function hslToHex(h,s=100,l=60){ const c=(1-Math.abs(2*l/100-1))*(s/100),x=c*(1-Math.abs((h/60)%2-1)),m=l/100-c/2; let r=0,g=0,b=0; if(0<=h&&h<60){r=c;g=x;} else if(60<=h&&h<120){r=x;g=c;} else if(120<=h&&h<180){g=c;b=x;} else if(180<=h&&h<240){g=x;b=c;} else if(240<=h&&h<300){r=x;b=c;} else{r=c;b=x;} const to=v=>('0'+Math.round((v+m)*255).toString(16)).slice(-2); return `#${to(r)}${to(g)}${to(b)}`; }
function initGradientEditor(){
  if(gWired) return; gWired=true;
  const hue=document.getElementById('hueBar'), ctx=hue.getContext('2d'), overlay=document.getElementById('gradModal');
  const paintHue=()=>{ const {width:w,height:h}=hue; const grd=ctx.createLinearGradient(0,0,w,0); for(let i=0;i<=360;i+=6){ grd.addColorStop(i/360,hslToHex(i,100,50)); } ctx.fillStyle=grd; ctx.fillRect(0,0,w,h); }; paintHue();
  const s1=document.getElementById('stop1'), s2=document.getElementById('stop2'), sample=document.getElementById('gradSample');
  s1.style.left=(gState.p1||20)+'%'; s2.style.left=(gState.p2||80)+'%';
  const posToHue=x=>Math.max(0,Math.min(1,x/hue.getBoundingClientRect().width))*360;
  function updateFromStops(){ const r=hue.getBoundingClientRect(); const x1=parseFloat(s1.style.left)/100*r.width, x2=parseFloat(s2.style.left)/100*r.width; gState.a=hslToHex(posToHue(x1),100,60); gState.b=hslToHex(posToHue(x2),100,60); updateGradSample(); gState.p1=parseFloat(s1.style.left); gState.p2=parseFloat(s2.style.left); }
  function updateGradSample(){ sample.style.background=`linear-gradient(135deg, ${gState.a}, ${gState.b})`; }
  let drag=null; function onDown(e){ const t=e.target; if(t===s1||t===s2){ drag=t; drag.style.cursor='grabbing'; e.preventDefault(); } }
  function onMove(e){ if(!drag) return; const r=hue.getBoundingClientRect(); const x=(e.touches?e.touches[0].clientX:e.clientX)-r.left; const pct=Math.max(0,Math.min(100,(x/r.width)*100)); drag.style.left=pct+'%'; updateFromStops(); }
  function onUp(){ if(!drag) return; drag.style.cursor='grab'; drag=null; }
  hue.addEventListener('mousedown',onDown); hue.addEventListener('touchstart',onDown);
  ...
  hue.addEventListener('click',(e)=>{ const r=hue.getBoundingClientRect(); const x=(e.clientX-r.left)/r.width*100; const t=Math.abs(x-parseFloat(s1.style.left))<Math.abs(x-parseFloat(s2.style.left))?s1:s2; t.style.left=Math.max(0,Math.min(100,x))+'%'; updateFromStops(); });
  updateGradSample();
}
